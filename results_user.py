...
def shResults(app):
    ...
    label1 = app.labelTxt(app.mainFr, f"Experiencia: {row[0]} XP")
    label1.pack(pady = 5)
    
    label2 = app.labelTxt(app.mainFr, f"Presición: {row[1]:.1f}%")
    label2.pack(pady = 5)
    ...
    canva.configure(yscrollcommand = sc.set, height = 300, width = obj.root.winfo_width() - 200)

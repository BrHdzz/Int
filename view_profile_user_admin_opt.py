...
def viewUsers_adminOpt(user, app):
    row = db.profileUsers_admin(user)
    ...
    b1.configure(command = lambda:db.getAdv(user))
    b1.pack(pady = 5, padx = 100, fill = "x")
    
    b2 = app.buttons(app.mainFr, "Ver resultados")
    b2.configure(command = lambda:view_results_user_admin.shResults(app, user))
    b2.pack(pady = 5, padx = 100, fill = "x")
    
    b3 = app.buttons(app.mainFr, "Ver donaciones")
    b3.configure(command = lambda:view_donation_user_admin.shDonate(app, user))
    b3.pack(pady = 5, padx = 100, fill = "x")
    
    b4 = app.buttons(app.mainFr, "Ver comentarios")
    b4.configure(command = lambda:view_coment_user_admin.shComent(app, user))
    b4.pack(pady = 5, padx = 100, fill = "x")
    
    b5 = app.buttons(app.mainFr, "Eliminar cuenta")
    b5.configure(command = lambda:db.deleteUser(app, user))

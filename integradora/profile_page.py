...
def profilePage(app, name, username, email):
    ...
    singin.config(width = 20)
    singin.grid(row = 0, column = 0, padx = 20)'''

    singin = app.buttons(buttonFr, "Cerrar Sesión")
    singin.config(width = 30, command = lambda:db.logout(app))
    singin.grid(row = 0, column = 0, padx = 30)

    back = app.buttons(buttonFr, "Atrás")
    back.config(width = 30, command = lambda:dashboard_page.dasboardPage(app))

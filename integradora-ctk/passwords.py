...
def psswd(app):
    ...
    canva.configure(yscrollcommand = sc.set, height = 250, width = obj.root.winfo_width() - 200)
    ...
    passw = app.inputs(frPass)
    passw.pack(ipadx = 10, ipady = 5, padx = 5, fill = "x")
    
    send = app.buttons(app.mainFr, "Publicar")
    send.configure(command = lambda:db.insert_password(passw.get(), app))
    send.pack(pady = 5, fill = "x", padx = 100)

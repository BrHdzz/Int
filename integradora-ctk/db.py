...
def login(user, passwd, app):
    try:
        ...
        cur.execute("select id, username, passwd, role, strike, date_kick from user where username = %s", (user,))
        ...
        if not user or not passwd:
            ...
            if bcrypt.checkpw(passwd.encode("utf-8"), row[2].encode("utf-8")):
                if row[3] == 9999:
...
def user_info(app):
    if os.path.exists("session.json"):
        ...
        try:
            ...
            cur.execute("select name, username, email, strike, id from user where id = %s", (id_user,))
            ...
            if row is None:
                ...
                profile_page.profilePage(app, user_name, user_username, user_email, user_strike, user_id)
    ...
    try:
        ...
        if row:
            for i in row:
                ...
                if i[0] == 0:
                    tk.Label(fr, text = f"{i[1]}", bg = "#000000", fg = "#ffffff", font = ("Inter", var_config.fontSizeText), width = obj.root.winfo_reqwidth() - 675, anchor = "w", justify = "left").pack()
                elif i[0] == 1:
                    tk.Label(fr, text = f"{i[1]}", bg = "#000000", fg = "#555555", font = ("Inter", var_config.fontSizeText, "italic"), width = obj.root.winfo_reqwidth() - 675, anchor = "w", justify = "left").pack()
        else:
        ...
        if row:
            for i in range(len(row)):
                ...
                if row[i][3] == 0:
                    ...
                    btn[i].configure(command = lambda id = row[i][0]:deleteComents(id, app))
                    btn[i].grid(padx = 5, row = 0, column = 1)

                    btn2.append(app.buttons(fr, "Ver perfil"))
                    btn2[i].configure(command = lambda id = row[i][5]:view_profile_user_admin.viewUsers_admin(id, app))
    ...
    try:
        ...
        for i in range(len(row)):
            if row[i][2] == 0:
                ...
                btn[j].configure(command = lambda id = row[i][0]:view_profile_user_admin.viewUsers_admin(id, app))
...
def profileUsers_admin(id_user):
    try:
        ...
        cur.execute("select u.id, u.name, u.username, u.date_, u.email, u.strike, p.xp, p.accurate from user u inner join progress p on (p.id_user = %s) where u.id = %s", (id_user, idu))
...
def getAdv(id):
    try:
        ...
        cur.execute("select strike from user where id = %s", (id,))

        row = cur.fetchone()

        if row:
            if row[0] + 1 == 2:
                cur.execute("update user set strike = %s, date_kick = %s where id = %s", ((row[0] + 1), (datetime.today() + timedelta(days = 7)), id))
            else:
                cur.execute("update user set strike = %s where id = %s", ((row[0] + 1), id))
        ...
        if row:
            for i in range(len(row)):
                ...
                if row[i][2] == 0:
                    ...
                    btn.append(app.buttons(fr, "Eliminar"))
                    btn[i].configure(command = lambda id = row[i][3]:deleteComents(id, app))
                    btn[i].grid(padx = 5, row = 0, column = 1)
                elif row[i][2] == 1:
...
def getRes(app, parent):
    if os.path.exists("session.json"):
        ...
        try:
            ...
            cur.execute("select r.xp, r.accurate, r.date_, a.name from result r inner join activity a on (a.id = r.id_activity) where id_user = %s", (id_user,))
...
def getProgress():
    if os.path.exists("session.json"):
        ...
        try:
            ...
            cur.execute("select xp, accurate from progress where id_user = %s", (id_user,))
...
def insertResultAct(xp, acc, id_a):
    if os.path.exists("session.json"):
        ...
        try:
            ...
            cur.execute("insert into result (xp, accurate, date_, id_user, id_activity) values (%s, %s, %s, %s, %s)", (xp, acc, datetime.now().date(), id_user, id_a))

            cur.execute("select id, accurate, xp from progress where id_user = %s", (id_user,))

            row = cur.fetchone()

            if row:
                if row[1] == 0:
                    cur.execute("update progress set accurate = %s, xp = %s where id_user = %s", (acc, (row[2] + xp), id_user))
                else:
                    cur.execute("update progress set accurate = %s, xp = %s where id_user = %s", (((row[1] + acc) / 2), (row[2] + xp), id_user))
